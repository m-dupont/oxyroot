...
fn generate_class(dump: &str) -> Result<Vec<ClassStreamerStrings>> {
    ...
    let mut GetStreamerInfos = Vec::new();
    let mut Element = Vec::new();
    let mut what = WhatIAmReading::Header;

    #[derive(Debug, Default)]
    struct CurrentClass<'a> {
        GetStreamerInfo: Vec<&'a str>,
        GetElements: Vec<Vec<&'a str>>,
    ...
    for line in dump.split("\n") {
        ...
        if line.starts_with(';') {
            ...
            what = match what {
                WhatIAmReading::Header => {
                    assert!(line.starts_with(";GetStreamerInfo"));
                    current_class_name_point_virgule = l;
                    WhatIAmReading::StreamerInfo
                }
                WhatIAmReading::StreamerInfo => {
                    assert!(line.starts_with(";GetElements"));
        ...
        match what {
            WhatIAmReading::Header => {
                if !current_class.GetStreamerInfo.is_empty() {
                    // current_class.current_class_name_point_virgule =
                    //     current_class_name_point_virgule;
                    if !Element.is_empty() {
                        current_class.GetElements.push(Element);
                        Element = Vec::new();
                    }
                    GetStreamerInfos.push(current_class);
            ...
            WhatIAmReading::StreamerInfo => {
                ...
                current_class.GetStreamerInfo.push(line);
            }
            WhatIAmReading::Elements => {
                if line.starts_with("==>") {
                    if !Element.is_empty() {
    ...
    for current_class in GetStreamerInfos.into_iter() {
        trace!(
            ";generate_class.current_class.name:{:?}",
            current_class.GetStreamerInfo.first()
        );
        let mut c = ClassStreamerStrings::default();

        c.class = generate_class_strings(current_class.GetStreamerInfo);
        ...
        c.elements = generate_elements_strings(current_class.GetElements);
        ret.push(c);
    }
...
pub fn populate_db(db: &mut DbStreamer, dump: &'static str) -> Result<()> {
    ...
    for class_str in classes_str {
        let class_name = &class_str.class.fName;
        ...
        let mut streamer_info = StreamerInfo::new(&class.fName, chksum, rvers);
        // let he = titles.get(class_name as &str);

        for element_str in elements {
            let name = &element_str.fName;
...
fn make_titles() -> HashMap<&'static str, HashMap<&'static str, HashMap<&'static str, &'static str>>>
{
    ...
    hh.insert("TTree", he);
    // TObjArray
    //si =  {'properties': {'fName': 'TObjArray'}, 'elements': {'TSeqCollection': {'name': 'TSeqCollection', 'title': 'Sequenceable collection ABC'}, 'fLowerBound': {'name': 'fLowerBound', 'title': 'Lower bound of the array'}, 'fLast': {'title': 'Last element in array containing an object', 'name': 'fLast'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TAttCanvas
    //si =  {'properties': {'fName': 'TAttCanvas'}, 'elements': {'fYdate': {'title': 'X position where to draw the date', 'name': 'fYdate'}, 'fAdate': {'name': 'fAdate', 'title': 'Alignment for the date'}, 'fTitleFromTop': {'name': 'fTitleFromTop', 'title': 'Y distance of Global Title from top'}, 'fXBetween': {'title': 'X distance between pads', 'name': 'fXBetween'}, 'fXdate': {'name': 'fXdate', 'title': 'X position where to draw the date'}, 'fYBetween': {'title': 'Y distance between pads', 'name': 'fYBetween'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TH1D
    //si =  {'elements': {'TH1': {'title': '1-Dim histogram base class', 'name': 'TH1'}, 'TArrayD': {'name': 'TArrayD', 'title': 'Array of doubles'}}, 'properties': {'fName': 'TH1D'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    hh.insert("TUUID", he);
    // TLeafObject
    //si =  {'properties': {'fName': 'TLeafObject'}, 'elements': {'TLeaf': {'name': 'TLeaf', 'title': 'Leaf: description of a Branch data type'}, 'fVirtual': {'title': 'Support for polymorphism, when set classname is written with object.', 'name': 'fVirtual'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TArrayD
    //si =  {'properties': {'fName': 'TArrayD'}, 'elements': {'fArray': {'name': 'fArray', 'title': '[fN] Array of fN doubles'}, 'TArray': {'name': 'TArray', 'title': 'Abstract array base class'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TLeafF16
    //si =  {'elements': {'TLeaf': {'name': 'TLeaf', 'title': 'Leaf: description of a Branch data type'}, 'fMinimum': {'name': 'fMinimum', 'title': 'Minimum value if leaf range is specified'}, 'fMaximum': {'title': 'Maximum value if leaf range is specified', 'name': 'fMaximum'}}, 'properties': {'fName': 'TLeafF16'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Leaf: description of a Branch data type");
    he.insert("TLeaf", h);
    ...
    // TLeafS
    //si =  {'properties': {'fName': 'TLeafS'}, 'elements': {'TLeaf': {'name': 'TLeaf', 'title': 'Leaf: description of a Branch data type'}, 'fMinimum': {'title': 'Minimum value if leaf range is specified', 'name': 'fMinimum'}, 'fMaximum': {'title': 'Maximum value if leaf range is specified', 'name': 'fMaximum'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Leaf: description of a Branch data type");
    he.insert("TLeaf", h);
    ...
    hh.insert("TLeafS", he);
    // TProfile
    //si =  {'properties': {'fName': 'TProfile'}, 'elements': {'fYmin': {'name': 'fYmin', 'title': 'Lower limit in Y (if set)'}, 'fTsumwy': {'name': 'fTsumwy', 'title': 'Total Sum of weight*Y'}, 'fBinEntries': {'title': 'number of entries per bin', 'name': 'fBinEntries'}, 'fYmax': {'name': 'fYmax', 'title': 'Upper limit in Y (if set)'}, 'fTsumwy2': {'title': 'Total Sum of weight*Y*Y', 'name': 'fTsumwy2'}, 'fErrorMode': {'name': 'fErrorMode', 'title': 'Option to compute errors'}, 'TH1D': {'name': 'TH1D', 'title': '1-Dim histograms (one double per channel)'}, 'fBinSumw2': {'title': 'Array of sum of squares of weights per bin', 'name': 'fBinSumw2'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TArrayL
    //si =  {'properties': {'fName': 'TArrayL'}, 'elements': {'TArray': {'title': 'Abstract array base class', 'name': 'TArray'}, 'fArray': {'name': 'fArray', 'title': '[fN] Array of fN longs'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Abstract array base class");
    he.insert("TArray", h);
    ...
    hh.insert("TF1", he);
    // TAttBBox2D
    //si =  {'properties': {'fName': 'TAttBBox2D'}}
    let mut he = HashMap::new();
    hh.insert("TAttBBox2D", he);
    // TDirectory
    //si =  {'properties': {'fName': 'TDirectory'}, 'elements': {'fMother': {'title': 'pointer to mother of the directory', 'name': 'fMother'}, 'TNamed': {'title': 'The basis for a named object (name, title)', 'name': 'TNamed'}, 'fUUID': {'title': 'Unique identifier', 'name': 'fUUID'}, 'fList': {'title': 'List of objects in memory', 'name': 'fList'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TH1F
    //si =  {'properties': {'fName': 'TH1F'}, 'elements': {'TH1': {'name': 'TH1', 'title': '1-Dim histogram base class'}, 'TArrayF': {'title': 'Array of floats', 'name': 'TArrayF'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "1-Dim histogram base class");
    he.insert("TH1", h);
    ...
    hh.insert("TH1F", he);
    // TLeafC
    //si =  {'properties': {'fName': 'TLeafC'}, 'elements': {'fMaximum': {'name': 'fMaximum', 'title': 'Maximum value if leaf range is specified'}, 'fMinimum': {'name': 'fMinimum', 'title': 'Minimum value if leaf range is specified'}, 'TLeaf': {'title': 'Leaf: description of a Branch data type', 'name': 'TLeaf'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Maximum value if leaf range is specified");
    he.insert("fMaximum", h);
    ...
    hh.insert("TLeafC", he);
    // TStreamerBasicType
    //si =  {'properties': {'fName': 'TStreamerBasicType'}, 'elements': {'TStreamerElement': {'title': 'Base class for one element (data member) to be Streamed', 'name': 'TStreamerElement'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert(
        "title",
    ...
    // TVector3
    //si =  {'elements': {'fZ': {'title': '', 'name': 'fZ'}, 'TObject': {'name': 'TObject', 'title': 'Basic ROOT object'}, 'fX': {'title': '', 'name': 'fX'}, 'fY': {'name': 'fY', 'title': ''}}, 'properties': {'fName': 'TVector3'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "");
    he.insert("fZ", h);
    ...
    // TList
    //si =  {'elements': {'TSeqCollection': {'title': 'Sequenceable collection ABC', 'name': 'TSeqCollection'}}, 'properties': {'fName': 'TList'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Sequenceable collection ABC");
    he.insert("TSeqCollection", h);
    hh.insert("TList", he);
    // TLeafElement
    //si =  {'elements': {'fType': {'title': 'leaf type', 'name': 'fType'}, 'TLeaf': {'name': 'TLeaf', 'title': 'Leaf: description of a Branch data type'}, 'fID': {'name': 'fID', 'title': 'element serial number in fInfo'}}, 'properties': {'fName': 'TLeafElement'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TH2D
    //si =  {'properties': {'fName': 'TH2D'}, 'elements': {'TArrayD': {'title': 'Array of doubles', 'name': 'TArrayD'}, 'TH2': {'name': 'TH2', 'title': '2-Dim histogram base class'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Array of doubles");
    he.insert("TArrayD", h);
    ...
    hh.insert("TH2D", he);
    // TF1Parameters
    //si =  {'properties': {'fName': 'TF1Parameters'}, 'elements': {'fParameters': {'title': 'parameter values', 'name': 'fParameters'}, 'fParNames': {'name': 'fParNames', 'title': 'parameter names'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TBits
    //si =  {'elements': {'TObject': {'name': 'TObject', 'title': 'Basic ROOT object'}, 'fNbits': {'title': 'Highest bit set + 1', 'name': 'fNbits'}, 'fAllBits': {'name': 'fAllBits', 'title': '[fNbytes] array of UChars'}, 'fNbytes': {'name': 'fNbytes', 'title': 'Number of UChars in fAllBits'}}, 'properties': {'fName': 'TBits'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Basic ROOT object");
    he.insert("TObject", h);
    ...
    hh.insert("TBits", he);
    // TStreamerArtificial
    //si =  {'elements': {'TStreamerElement': {'title': 'Base class for one element (data member) to be Streamed', 'name': 'TStreamerElement'}}, 'properties': {'fName': 'TStreamerArtificial'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert(
        "title",
    ...
    hh.insert("TGraph", he);
    // TArrayI
    //si =  {'properties': {'fName': 'TArrayI'}, 'elements': {'fArray': {'name': 'fArray', 'title': '[fN] Array of fN 32 bit integers'}, 'TArray': {'title': 'Abstract array base class', 'name': 'TArray'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TClonesArray
    //si =  {'properties': {'fName': 'TClonesArray'}, 'elements': {'TObjArray': {'name': 'TObjArray', 'title': 'An array of objects'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TProcessID
    //si =  {'elements': {'TNamed': {'name': 'TNamed', 'title': 'The basis for a named object (name, title)'}}, 'properties': {'fName': 'TProcessID'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "The basis for a named object (name, title)");
    he.insert("TNamed", h);
    ...
    hh.insert("TLeaf", he);
    // TH1K
    //si =  {'properties': {'fName': 'TH1K'}, 'elements': {'TH1': {'title': '1-Dim histogram base class', 'name': 'TH1'}, 'fNIn': {'name': 'fNIn', 'title': ''}, 'TArrayF': {'name': 'TArrayF', 'title': 'Array of floats'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "1-Dim histogram base class");
    he.insert("TH1", h);
    ...
    hh.insert("TFile", he);
    // TNtuple
    //si =  {'elements': {'fNvar': {'name': 'fNvar', 'title': 'Number of columns'}, 'TTree': {'name': 'TTree', 'title': 'Tree descriptor (the main ROOT I/O class)'}}, 'properties': {'fName': 'TNtuple'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TF1AbsComposition
    //si =  {'properties': {'fName': 'TF1AbsComposition'}, 'elements': {'TObject': {'name': 'TObject', 'title': 'Basic ROOT object'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Basic ROOT object");
    he.insert("TObject", h);
    ...
    // THashList
    //si =  {'elements': {'TList': {'title': 'Doubly linked list', 'name': 'TList'}}, 'properties': {'fName': 'THashList'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // THashTable
    //si =  {'properties': {'fName': 'THashTable'}, 'elements': {'TCollection': {'title': 'Collection abstract base class', 'name': 'TCollection'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TDatime
    //si =  {'properties': {'fName': 'TDatime'}, 'elements': {'fDatime': {'name': 'fDatime', 'title': 'Date (relative to 1995) + time'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TArrayL64
    //si =  {'elements': {'TArray': {'title': 'Abstract array base class', 'name': 'TArray'}, 'fArray': {'title': '[fN] Array of fN long64s', 'name': 'fArray'}}, 'properties': {'fName': 'TArrayL64'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Abstract array base class");
    he.insert("TArray", h);
    ...
    // TH1S
    //si =  {'properties': {'fName': 'TH1S'}, 'elements': {'TH1': {'name': 'TH1', 'title': '1-Dim histogram base class'}, 'TArrayS': {'name': 'TArrayS', 'title': 'Array of shorts'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "1-Dim histogram base class");
    he.insert("TH1", h);
    ...
    // TCollection
    //si =  {'elements': {'fName': {'title': 'name of the collection', 'name': 'fName'}, 'fSize': {'title': 'number of elements in collection', 'name': 'fSize'}, 'TObject': {'name': 'TObject', 'title': 'Basic ROOT object'}}, 'properties': {'fName': 'TCollection'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TH2C
    //si =  {'elements': {'TArrayC': {'title': 'Array of chars', 'name': 'TArrayC'}, 'TH2': {'title': '2-Dim histogram base class', 'name': 'TH2'}}, 'properties': {'fName': 'TH2C'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    hh.insert("TH2C", he);
    // TBranchObject
    //si =  {'properties': {'fName': 'TBranchObject'}, 'elements': {'TBranch': {'name': 'TBranch', 'title': 'Branch descriptor'}, 'fClassName': {'title': 'Class name of referenced object', 'name': 'fClassName'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TAttMarker
    //si =  {'elements': {'fMarkerStyle': {'name': 'fMarkerStyle', 'title': 'Marker style'}, 'fMarkerSize': {'name': 'fMarkerSize', 'title': 'Marker size'}, 'fMarkerColor': {'name': 'fMarkerColor', 'title': 'Marker color'}}, 'properties': {'fName': 'TAttMarker'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TLeafI
    //si =  {'elements': {'TLeaf': {'name': 'TLeaf', 'title': 'Leaf: description of a Branch data type'}, 'fMaximum': {'title': 'Maximum value if leaf range is specified', 'name': 'fMaximum'}, 'fMinimum': {'name': 'fMinimum', 'title': 'Minimum value if leaf range is specified'}}, 'properties': {'fName': 'TLeafI'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Leaf: description of a Branch data type");
    he.insert("TLeaf", h);
    ...
    hh.insert("TLeafI", he);
    // TH2S
    //si =  {'properties': {'fName': 'TH2S'}, 'elements': {'TH2': {'title': '2-Dim histogram base class', 'name': 'TH2'}, 'TArrayS': {'name': 'TArrayS', 'title': 'Array of shorts'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "2-Dim histogram base class");
    he.insert("TH2", h);
    ...
    // TProcessUUID
    //si =  {'elements': {'TProcessID': {'name': 'TProcessID', 'title': 'Process Unique Identifier in time and space'}, 'fActive': {'title': 'Table of active UUIDs', 'name': 'fActive'}, 'fUUIDs': {'title': 'Global list of TUUIDs', 'name': 'fUUIDs'}}, 'properties': {'fName': 'TProcessUUID'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TStreamerObjectAny
    //si =  {'properties': {'fName': 'TStreamerObjectAny'}, 'elements': {'TStreamerElement': {'name': 'TStreamerElement', 'title': 'Base class for one element (data member) to be Streamed'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert(
        "title",
    ...
    // TStreamerSTL
    //si =  {'elements': {'fSTLtype': {'name': 'fSTLtype', 'title': 'type of STL vector'}, 'fCtype': {'title': 'STL contained type', 'name': 'fCtype'}, 'TStreamerElement': {'title': 'Base class for one element (data member) to be Streamed', 'name': 'TStreamerElement'}}, 'properties': {'fName': 'TStreamerSTL'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    //si =  {'properties': {'fName': 'TStreamerObjectPointer'}, 'elements': {'TStreamerElement': {'name': 'TStreamerElement', 'title': 'Base class for one element (data member) to be Streamed'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert(
        "title",
    ...
    // TStreamerBase
    //si =  {'elements': {'TStreamerElement': {'title': 'Base class for one element (data member) to be Streamed', 'name': 'TStreamerElement'}, 'fBaseVersion': {'name': 'fBaseVersion', 'title': 'version number of the base class (used during memberwise streaming)'}}, 'properties': {'fName': 'TStreamerBase'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert(
        "title",
    ...
    // TStreamerString
    //si =  {'properties': {'fName': 'TStreamerString'}, 'elements': {'TStreamerElement': {'name': 'TStreamerElement', 'title': 'Base class for one element (data member) to be Streamed'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert(
        "title",
    ...
    // TH1I
    //si =  {'elements': {'TArrayI': {'title': 'Array of ints', 'name': 'TArrayI'}, 'TH1': {'title': '1-Dim histogram base class', 'name': 'TH1'}}, 'properties': {'fName': 'TH1I'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    hh.insert("TH1I", he);
    // TStreamerSTLstring
    //si =  {'properties': {'fName': 'TStreamerSTLstring'}, 'elements': {'TStreamerSTL': {'title': 'Streamer element of type STL container', 'name': 'TStreamerSTL'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TLeafO
    //si =  {'elements': {'fMaximum': {'name': 'fMaximum', 'title': 'Maximum value if leaf range is specified'}, 'fMinimum': {'title': 'Minimum value if leaf range is specified', 'name': 'fMinimum'}, 'TLeaf': {'title': 'Leaf: description of a Branch data type', 'name': 'TLeaf'}}, 'properties': {'fName': 'TLeafO'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Maximum value if leaf range is specified");
    he.insert("fMaximum", h);
    ...
    hh.insert("TLeafO", he);
    // TObject
    //si =  {'properties': {'fName': 'TObject'}, 'elements': {'fUniqueID': {'name': 'fUniqueID', 'title': 'object unique identifier'}, 'fBits': {'name': 'fBits', 'title': 'bit field status word'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TH2
    //si =  {'properties': {'fName': 'TH2'}, 'elements': {'fTsumwy2': {'name': 'fTsumwy2', 'title': 'Total Sum of weight*Y*Y'}, 'fScalefactor': {'title': 'Scale factor', 'name': 'fScalefactor'}, 'fTsumwy': {'title': 'Total Sum of weight*Y', 'name': 'fTsumwy'}, 'TH1': {'name': 'TH1', 'title': '1-Dim histogram base class'}, 'fTsumwxy': {'name': 'fTsumwxy', 'title': 'Total Sum of weight*X*Y'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Total Sum of weight*Y*Y");
    he.insert("fTsumwy2", h);
    ...
    hh.insert("TH2", he);
    // TH2F
    //si =  {'elements': {'TArrayF': {'title': 'Array of floats', 'name': 'TArrayF'}, 'TH2': {'name': 'TH2', 'title': '2-Dim histogram base class'}}, 'properties': {'fName': 'TH2F'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Array of floats");
    he.insert("TArrayF", h);
    ...
    // TProfile2D
    //si =  {'properties': {'fName': 'TProfile2D'}, 'elements': {'fBinEntries': {'name': 'fBinEntries', 'title': 'Number of entries per bin'}, 'fBinSumw2': {'title': 'Array of sum of squares of weights per bin', 'name': 'fBinSumw2'}, 'fErrorMode': {'title': 'Option to compute errors', 'name': 'fErrorMode'}, 'TH2D': {'name': 'TH2D', 'title': '2-Dim histograms (one double per channel)'}, 'fZmax': {'title': 'Upper limit in Z (if set)', 'name': 'fZmax'}, 'fTsumwz': {'name': 'fTsumwz', 'title': 'Total Sum of weight*Z'}, 'fZmin': {'title': 'Lower limit in Z (if set)', 'name': 'fZmin'}, 'fTsumwz2': {'name': 'fTsumwz2', 'title': 'Total Sum of weight*Z*Z'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Number of entries per bin");
    he.insert("fBinEntries", h);
    ...
    // TObjString
    //si =  {'properties': {'fName': 'TObjString'}, 'elements': {'TObject': {'name': 'TObject', 'title': 'Basic ROOT object'}, 'fString': {'name': 'fString', 'title': 'wrapped TString'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Basic ROOT object");
    he.insert("TObject", h);
    ...
    // TNtupleD
    //si =  {'elements': {'fNvar': {'name': 'fNvar', 'title': 'Number of columns'}, 'TTree': {'name': 'TTree', 'title': 'Tree descriptor (the main ROOT I/O class)'}}, 'properties': {'fName': 'TNtupleD'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Number of columns");
    he.insert("fNvar", h);
    ...
    hh.insert("TChain", he);
    // TStreamerObject
    //si =  {'properties': {'fName': 'TStreamerObject'}, 'elements': {'TStreamerElement': {'name': 'TStreamerElement', 'title': 'Base class for one element (data member) to be Streamed'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert(
        "title",
    ...
    // TAttFill
    //si =  {'properties': {'fName': 'TAttFill'}, 'elements': {'fFillColor': {'name': 'fFillColor', 'title': 'Fill area color'}, 'fFillStyle': {'name': 'fFillStyle', 'title': 'Fill area style'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TH1C
    //si =  {'elements': {'TH1': {'title': '1-Dim histogram base class', 'name': 'TH1'}, 'TArrayC': {'title': 'Array of chars', 'name': 'TArrayC'}}, 'properties': {'fName': 'TH1C'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "1-Dim histogram base class");
    he.insert("TH1", h);
    ...
    hh.insert("TH1C", he);
    // TGraphErrors
    //si =  {'elements': {'TGraph': {'title': 'Graph graphics class', 'name': 'TGraph'}, 'fEY': {'name': 'fEY', 'title': '[fNpoints] array of Y errors'}, 'fEX': {'name': 'fEX', 'title': '[fNpoints] array of X errors'}}, 'properties': {'fName': 'TGraphErrors'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    hh.insert("TH1", he);
    // TGraphAsymmErrors
    //si =  {'properties': {'fName': 'TGraphAsymmErrors'}, 'elements': {'fEYlow': {'name': 'fEYlow', 'title': '[fNpoints] array of Y low errors'}, 'fEXhigh': {'name': 'fEXhigh', 'title': '[fNpoints] array of X high errors'}, 'fEXlow': {'name': 'fEXlow', 'title': '[fNpoints] array of X low errors'}, 'TGraph': {'name': 'TGraph', 'title': 'Graph graphics class'}, 'fEYhigh': {'name': 'fEYhigh', 'title': '[fNpoints] array of Y high errors'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TRef
    //si =  {'elements': {'TObject': {'name': 'TObject', 'title': 'Basic ROOT object'}}, 'properties': {'fName': 'TRef'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Basic ROOT object");
    he.insert("TObject", h);
    hh.insert("TRef", he);
    // TLorentzVector
    //si =  {'properties': {'fName': 'TLorentzVector'}, 'elements': {'TObject': {'name': 'TObject', 'title': 'Basic ROOT object'}, 'fE': {'name': 'fE', 'title': 'time or energy of (x,y,z,t) or (px,py,pz,e)'}, 'fP': {'name': 'fP', 'title': '3 vector component'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Basic ROOT object");
    he.insert("TObject", h);
    ...
    // TArrayS
    //si =  {'elements': {'fArray': {'name': 'fArray', 'title': '[fN] Array of fN shorts'}, 'TArray': {'name': 'TArray', 'title': 'Abstract array base class'}}, 'properties': {'fName': 'TArrayS'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TLeafB
    //si =  {'properties': {'fName': 'TLeafB'}, 'elements': {'fMaximum': {'title': 'Maximum value if leaf range is specified', 'name': 'fMaximum'}, 'TLeaf': {'name': 'TLeaf', 'title': 'Leaf: description of a Branch data type'}, 'fMinimum': {'title': 'Minimum value if leaf range is specified', 'name': 'fMinimum'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Maximum value if leaf range is specified");
    he.insert("fMaximum", h);
    ...
    hh.insert("TLeafB", he);
    // TVirtualPad
    //si =  {'properties': {'fName': 'TVirtualPad'}, 'elements': {'TAttPad': {'title': 'Pad attributes', 'name': 'TAttPad'}, 'TObject': {'title': 'Basic ROOT object', 'name': 'TObject'}, 'TQObject': {'title': 'Base class for object communication mechanism', 'name': 'TQObject'}, 'TAttFill': {'name': 'TAttFill', 'title': 'Fill area attributes'}, 'TAttLine': {'name': 'TAttLine', 'title': 'Line attributes'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TNamed
    //si =  {'properties': {'fName': 'TNamed'}, 'elements': {'fName': {'title': 'object identifier', 'name': 'fName'}, 'fTitle': {'name': 'fTitle', 'title': 'object title'}, 'TObject': {'name': 'TObject', 'title': 'Basic ROOT object'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    hh.insert("TAxis", he);
    // TArrayC
    //si =  {'properties': {'fName': 'TArrayC'}, 'elements': {'TArray': {'title': 'Abstract array base class', 'name': 'TArray'}, 'fArray': {'title': '[fN] Array of fN chars', 'name': 'fArray'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Abstract array base class");
    he.insert("TArray", h);
    ...
    // TLeafL
    //si =  {'elements': {'fMaximum': {'name': 'fMaximum', 'title': 'Maximum value if leaf range is specified'}, 'TLeaf': {'name': 'TLeaf', 'title': 'Leaf: description of a Branch data type'}, 'fMinimum': {'name': 'fMinimum', 'title': 'Minimum value if leaf range is specified'}}, 'properties': {'fName': 'TLeafL'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Maximum value if leaf range is specified");
    he.insert("fMaximum", h);
    ...
    hh.insert("TLeafL", he);
    // TString
    //si =  {'properties': {'fName': 'TString'}}
    let mut he = HashMap::new();
    hh.insert("TString", he);
    // TH2PolyBin
    //si =  {'properties': {'fName': 'TH2PolyBin'}, 'elements': {'fContent': {'title': 'Bin content', 'name': 'fContent'}, 'fArea': {'title': 'Bin area', 'name': 'fArea'}, 'fPoly': {'name': 'fPoly', 'title': 'Object holding the polygon definition'}, 'fNumber': {'name': 'fNumber', 'title': 'Bin number of the bin in TH2Poly'}, 'fYmin': {'title': 'Y minimum value', 'name': 'fYmin'}, 'fChanged': {'name': 'fChanged', 'title': 'For the 3D Painter'}, 'fYmax': {'name': 'fYmax', 'title': 'Y maximum value'}, 'fXmax': {'title': 'X maximum value', 'name': 'fXmax'}, 'TObject': {'name': 'TObject', 'title': 'Basic ROOT object'}, 'fXmin': {'name': 'fXmin', 'title': 'X minimum value'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TMap
    //si =  {'properties': {'fName': 'TMap'}, 'elements': {'TCollection': {'name': 'TCollection', 'title': 'Collection abstract base class'}, 'fTable': {'title': "Hash table used to store TPair's", 'name': 'fTable'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Collection abstract base class");
    he.insert("TCollection", h);
    ...
    hh.insert("TPad", he);
    // TAttLine
    //si =  {'elements': {'fLineColor': {'title': 'Line color', 'name': 'fLineColor'}, 'fLineStyle': {'name': 'fLineStyle', 'title': 'Line style'}, 'fLineWidth': {'name': 'fLineWidth', 'title': 'Line width'}}, 'properties': {'fName': 'TAttLine'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TLimit
    //si =  {'properties': {'fName': 'TLimit'}}
    let mut he = HashMap::new();
    hh.insert("TLimit", he);
    // TBranchRef
    //si =  {'elements': {'fRefTable': {'name': 'fRefTable', 'title': 'pointer to the TRefTable'}, 'TBranch': {'title': 'Branch descriptor', 'name': 'TBranch'}}, 'properties': {'fName': 'TBranchRef'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    // TLeafD32
    //si =  {'elements': {'fMaximum': {'name': 'fMaximum', 'title': 'Maximum value if leaf range is specified'}, 'fMinimum': {'name': 'fMinimum', 'title': 'Minimum value if leaf range is specified'}, 'TLeaf': {'title': 'Leaf: description of a Branch data type', 'name': 'TLeaf'}}, 'properties': {'fName': 'TLeafD32'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Maximum value if leaf range is specified");
    he.insert("fMaximum", h);
    ...
    // TSeqCollection
    //si =  {'elements': {'TCollection': {'name': 'TCollection', 'title': 'Collection abstract base class'}}, 'properties': {'fName': 'TSeqCollection'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Collection abstract base class");
    he.insert("TCollection", h);
    ...
    // ROOT
    //si =  [':TIOFeatures.properties.fName:ROOT::TIOFeatures', ':TIOFeatures.elements.fIOBits.name:fIOBits', ':TIOFeatures.elements.fIOBits.title:']
    // TLeafF
    //si =  {'elements': {'TLeaf': {'name': 'TLeaf', 'title': 'Leaf: description of a Branch data type'}, 'fMaximum': {'name': 'fMaximum', 'title': 'Maximum value if leaf range is specified'}, 'fMinimum': {'title': 'Minimum value if leaf range is specified', 'name': 'fMinimum'}}, 'properties': {'fName': 'TLeafF'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Leaf: description of a Branch data type");
    he.insert("TLeaf", h);
    ...
    hh.insert("TLeafF", he);
    // TArrayF
    //si =  {'properties': {'fName': 'TArrayF'}, 'elements': {'TArray': {'title': 'Abstract array base class', 'name': 'TArray'}, 'fArray': {'title': '[fN] Array of fN floats', 'name': 'fArray'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Abstract array base class");
    he.insert("TArray", h);
    ...
    // TLeafD
    //si =  {'elements': {'fMaximum': {'title': 'Maximum value if leaf range is specified', 'name': 'fMaximum'}, 'TLeaf': {'title': 'Leaf: description of a Branch data type', 'name': 'TLeaf'}, 'fMinimum': {'title': 'Minimum value if leaf range is specified', 'name': 'fMinimum'}}, 'properties': {'fName': 'TLeafD'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Maximum value if leaf range is specified");
    he.insert("fMaximum", h);
    ...
    //si =  {'elements': {'TStreamerElement': {'title': 'Base class for one element (data member) to be Streamed', 'name': 'TStreamerElement'}}, 'properties': {'fName': 'TStreamerObjectAnyPointer'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert(
        "title",
    ...
    // TLeafG
    //si =  {'elements': {'fMinimum': {'title': 'Minimum value if leaf range is specified', 'name': 'fMinimum'}, 'TLeaf': {'name': 'TLeaf', 'title': 'Leaf: description of a Branch data type'}, 'fMaximum': {'name': 'fMaximum', 'title': 'Maximum value if leaf range is specified'}}, 'properties': {'fName': 'TLeafG'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Minimum value if leaf range is specified");
    he.insert("fMinimum", h);
    ...
    // TVector2
    //si =  {'properties': {'fName': 'TVector2'}, 'elements': {'fY': {'name': 'fY', 'title': ''}, 'TObject': {'name': 'TObject', 'title': 'Basic ROOT object'}, 'fX': {'name': 'fX', 'title': 'components of the vector'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "");
    he.insert("fY", h);
    ...
    // TArray
    //si =  {'elements': {'fN': {'name': 'fN', 'title': 'Number of array elements'}}, 'properties': {'fName': 'TArray'}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    ...
    hh.insert("TArray", he);
    // TH2I
    //si =  {'properties': {'fName': 'TH2I'}, 'elements': {'TArrayI': {'name': 'TArrayI', 'title': 'Array of ints'}, 'TH2': {'name': 'TH2', 'title': '2-Dim histogram base class'}}}
    let mut he = HashMap::new();
    let mut h = HashMap::new();
    h.insert("title", "Array of ints");
    he.insert("TArrayI", h);
